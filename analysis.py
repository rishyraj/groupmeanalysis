...
class GroupMeStatistics:    
    ...
    def getPersonData(self,name):
        idToName = self.userIdToName()
        if (self.isValidUser(name,idToName)==False):
    ...
    def getGeneralDayData(self,date):
        wordDict = {}
        texts = []
        for messages in self.data:
    ...
    def getPersonDayData(self,name,date):
        idToName = self.userIdToName()
        if (self.isValidUser(name,idToName)==False):
            print("NameNotFoundError: This person was not in the group chat.")
            return "Error"   
    ...
    def userIdToName(self):
        idToName = {}
        nameToId = {}
        for i in range(len(self.data)-1,-1,-1):
            messages = self.data[i]
    ...
    def isValidUser(self,name,idToName):
        names = set(idToName.values())
        return (name in names)

    def mostCommon(self,n,words):
    ...
    def wordRange(self,low,high,words):
        ordered = sorted(words.items(), key=lambda x:x[1],reverse=True)
        num = high - low + 1
        x=np.arange(num)
        x_ticks = []
    ...
    def wordConcentration(self, personData,genData,tag):
        personCt = self.lookupWord(tag,personData)
        genCt = self.lookupWord(tag,genData)
        if (genCt==False):
            print("=======")
    ...
    def plotTimeData(self, timeData,beginningDate=False,endDate=False):
        dates = timeData[0]
    ...
    def getUserActivityRank(self,idToName):
    ...
    def getMostActiveUsers(self, persons,n):
        activeUsers = []
        x=np.arange(n)
        x_ticks = []
        y_val = []
        messageCt = self.messagesSentCount(persons)
    ...
    def getActiveUsersRange(self, low,high,persons):
        activeUsers = []
        num = high - low + 1
        x=np.arange(num)
        x_ticks = []
    ...
    def lookupUserActivityStats(self, person,users,idToName):
        if (self.isValidUser(person,idToName)==False):
            print("NameNotFoundError: This person was not in the group chat.")
            return "Error"
        count = 0
        messageCt = self.messagesSentCount(users)
        for user in users:
            count+=1
            if (user[0]==person):
    ...
    def reverseLookup(self, n,users):
        try:
            user = users[n-1]
    ...
    def messagesSentCount(self, users):
        count=0
        for user in users:
            count+=user[1]
        return count

    def pickleData(self, filename, words):
        fn = filename+'.pickle'
        file = open(fn,'wb')
        pickle.dump(words,file, protocol=pickle.HIGHEST_PROTOCOL)
        file.close()

    def loadPickleData(self, filepath):
...
#=======================================================================================================================
groupMeStat = GroupMeStatistics('message.json')
# print(groupMeStat.getData())
# words_gen = groupMeStat.getGeneralData()
# groupMeStat.mostCommon(5,words_gen)
